...
struct stream_out {
	...
	*audio
	*/
	long time_delay_host;
	bool time_delay_host_flag;
	long time_delay_slave;
	bool time_delay_slave_flag;
	int logcount;
	size_t mCount;
	CblkMemory* mem1;
	void* mem2;
	...
	bool standbyflag;
	bool exitflag;
	void (*write_I)(struct audio_stream_out *stream, void* buffer, size_t bytes);
	void (*standby_I)(struct audio_stream *stream);
	//int (*setstartflag_I)(struct audio_stream *stream,int flag);
};
