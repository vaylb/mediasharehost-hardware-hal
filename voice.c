...
int voice_stop_usecase(struct audio_device *adev, audio_usecase_t usecase_id)
{
    ...
    // ALOGD("%s: enter usecase:%s", __func__, use_case_table[usecase_id]); //vaylb disabled this avoid compile error
...
int voice_start_usecase(struct audio_device *adev, audio_usecase_t usecase_id)
{
    ...
    // ALOGD("%s: enter usecase:%s", __func__, use_case_table[usecase_id]); //vaylb disabled this to avoid compile error
...
void voice_init(struct audio_device *adev)
{
    ...
    for (i = 0; i < MAX_VOICE_SESSIONS; i++) {
        ...
        adev->voice.session[i].state.new_t = CALL_INACTIVE;
...
void voice_update_devices_for_all_voice_usecases(struct audio_device *adev)
{
    ...
    list_for_each(node, &adev->usecase_list) {
        ...
        if (usecase->type == VOICE_CALL) {
            ...
            //       use_case_table[usecase->id]);  //vaylb disabled this to avoid compile error
