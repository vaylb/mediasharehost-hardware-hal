...
//audio:for control the standby process
static int out_standby_new(struct audio_stream *stream)
{
	ALOGE("audio_test-->HAL:out_standby_new run.");
	struct stream_out *out = (struct stream_out *)stream;
	if(out->startflag == 0) return out_standby(stream);
	else {
		out->standbyflag = true;
    	return 0;
	}
}
...
static int out_set_parameters(struct audio_stream *stream, const char *kvpairs)
{
	...
	/**
	*saudio
	*add "if" sentence to change exitflag while exit.
	*/
	if((out->startflag == true)&&(strcmp(kvpairs,"exiting=1")==0)) out->exitflag = true;
...
static size_t availableToWrite(CblkMemory* cblk)
{
    return cblk->mMaxFrames - (cblk->mRear - android_atomic_acquire_load(&cblk->mFront));
}

static size_t data_transfer(struct stream_out *out, void* buffer, size_t bytes){
		size_t count = bytes >> 2;
		size_t total = 0;
		long sleepNs;
		//pthread_mutex_lock(&out->lock);
		while (count > 0) {
			if(out->startflag == 0) return bytes;
			
			size_t avail = availableToWrite(out->mem1);
			if(avail < out->mCount){
				sleepNs = 1000000; //1ms
			...
			size_t rear = out->mem1->mRear % out->mem1->mMaxFrames;
			size_t part1 = out->mem1->mMaxFrames - rear;
			if (part1 > written) {
				part1 = written;
			}
	
			if (part1 > 0) {
				memcpy((char*)out->mem1->mBuffer + (rear << 2), buffer, part1 << 2);
					
				if (rear + part1 == out->mem1->mMaxFrames) {
					size_t part2 = written - part1;
					if (part2 > 0) {
						memcpy(out->mem1->mBuffer, (char*)buffer + (part1 << 2), part2 << 2);
					}
				}
				android_atomic_release_store(written + out->mem1->mRear, &out->mem1->mRear);
			}
			count -= written;
			total += written;
			buffer = (char*)buffer + (written << 2);
		}
		//pthread_mutex_unlock(&out->lock);
		return total<<2;
}
...
static ssize_t out_write_new(struct audio_stream_out *stream, void* buffer, size_t bytes)
{
    struct stream_out *out = (struct stream_out *)stream;
    struct audio_device *adev = out->dev;
	if(out->startflag==0) return out_write(stream,buffer,bytes);
	else if(out->startflag == 1){
		if(out->standbyflag==true) out->standbyflag = false;
		return data_transfer(out, buffer, bytes);
	}
	return bytes;
}
                                   ...
                                   const char *address __unused)
{
    ...
    *stream_out = NULL;
	...
	out->stream.common.standby = out_standby_new; //audio:changed from out_standby to out_standby_new; 
	...
	out->stream.write = out_write_new; //audio:changed from out_write to out_write_new;
    out->stream.get_render_position = out_get_render_position;
    out->stream.get_next_write_timestamp = out_get_next_write_timestamp;
    out->stream.get_presentation_position = out_get_presentation_position;

	out->write_I = out_write;  //point to out_write;
	out->standby_I = out_standby; //point to out_standby;
	//out->setstartflag_I = out_setstartflag;
